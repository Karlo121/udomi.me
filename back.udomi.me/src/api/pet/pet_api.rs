...
impl PetApi {
    ...
    async fn create_pet(&self, pool: Data<&PgPool>, mut multipart: Multipart) -> Result<Json<Pet>> {
        ...
        let image_url = if let Some(image) = image_data {
            let file_name = format!("{}.jpg", Utc::now().timestamp()); // Or another unique identifier
        ...
        let new_pet = sqlx::query_as!(
            Pet,
            r#"
            INSERT INTO pets (name, breed_id, age, description, gender, adopted, created_at, created_by, image_url)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
            RETURNING id, name, breed_id, age, description, gender as "gender: Gender", adopted, created_at, created_by, image_url
            "#,
            create_pet.name,
            create_pet.breed_id,
            create_pet.age,
    ...
    async fn get_pet_by_id(&self, pool: Data<&PgPool>, id: Path<i32>) -> Result<Json<Pet>> {
        let pet = sqlx::query_as!(
            Pet,
            r#"
            SELECT id, name, breed_id, age, description, gender as "gender: Gender", adopted, created_at, created_by, image_url
